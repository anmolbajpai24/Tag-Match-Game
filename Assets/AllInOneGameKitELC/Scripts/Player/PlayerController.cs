...
public class PlayerController : MonoBehaviour
{
	...
	void GettingRotationDirection () {
		...
		directionVector = new Vector3(joystick.Horizontal, 0, joystick.Vertical);
			...
			if (jumpedOffWallForWallJump){
				...
				if (noCollisionTimer >= 5 && !currentlyOnWall){
					...
					if (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis){
						wallJumpDirection += (joystick.Horizontal*(wallJumping.overallMovementSpeed/8) * playerCamera.transform.right);
						wallJumpDirection += (joystick.Vertical*(wallJumping.overallMovementSpeed/8) * playerCamera.transform.forward);
					}
					else {
						if (movement.sideScrolling.lockMovementOnXAxis){
							...
							if (!movement.sideScrolling.flipAxisRotation){
								if (transform.eulerAngles.y >= 270 && transform.eulerAngles.y <= 360 || transform.eulerAngles.y <= 90 && transform.eulerAngles.y >= -90){
									wallJumpDirection += (-joystick.Horizontal*(wallJumping.overallMovementSpeed/8) * transform.forward);
								}
								else {
									wallJumpDirection += (joystick.Horizontal*(wallJumping.overallMovementSpeed/8) * transform.forward);
								}
	...
	void ClimbingWall () {
		...
		if (currentlyClimbingWall && !pullingUp){
			...
			if (joystick.Horizontal > 0 && lastAxis <= 0 || joystick.Horizontal < 0 && lastAxis >= 0){
				switchedDirection = true;
			}
			lastAxis = joystick.Horizontal;
			
			//climbing down/off of ladder or wall
			if (joystick.Vertical >= 0 || switchedDirection && !downCanBePressed || !grounded.currentlyGrounded){
				downCanBePressed = true;
				if (joystick.Vertical > 0 || !grounded.currentlyGrounded){
			...
			if (joystick.Vertical < 0 && joystick.Horizontal == 0 && (!downCanBePressed || reachedBottomPoint && (!dropOffAtBottom2 || grounded.currentlyGrounded && !dropOffAtFloor2))){
			...
			if (directionVector.magnitude != 0){
				...
				if (climbHorizontally2 && climbVertically2 && (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis)){
					...
					if (stopAtSides2 && (reachedRightPoint && joystick.Horizontal > 0 || reachedLeftPoint && joystick.Horizontal < 0)
					&& (!downCanBePressed || reachedTopPoint && joystick.Vertical > 0 || reachedBottomPoint && joystick.Vertical < 0)){
						climbDirection = Vector3.zero;
					}
					//if we have reached the left or right point, do not allow horizontal movement in that direction
					else if (downCanBePressed && stopAtSides2 && (reachedRightPoint && joystick.Horizontal > 0 || reachedLeftPoint && joystick.Horizontal < 0)
					&& ((!reachedTopPoint || joystick.Vertical <= 0) && (!reachedBottomPoint || joystick.Vertical >= 0))){
						climbDirection = (joystick.Vertical*transform.up) * climbingMovement;
					}
					//if down cannot be pressed, or we have reached the top or bottom point, do not allow vertical movement in that direction
					else if (!downCanBePressed || reachedTopPoint && joystick.Vertical > 0 || reachedBottomPoint && joystick.Vertical < 0){
						climbDirection = (joystick.Horizontal*transform.right) * climbingMovement;
					}
					//if down can be pressed, and we have not reached the top, bottom, right, or left point, allow movement in every direction
						...
						climbDirection = (joystick.Horizontal*transform.right + joystick.Vertical*transform.up) * climbingMovement;
					}
				...
				//climbing vertically
					...
					if (!downCanBePressed || reachedTopPoint && joystick.Vertical > 0 || reachedBottomPoint && joystick.Vertical < 0){
						climbDirection = Vector3.zero;
					}
			...
			if (!pullingUp){
				...
				if (currentlyClimbingWall){
					if (climbHorizontally2 && (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis)){
						//if we have not reached the sides of the climbable object
						if (!stopAtSides2 || (!reachedRightPoint || joystick.Horizontal < 0) && (!reachedLeftPoint || joystick.Horizontal > 0)){
							animator.SetFloat("climbState", Mathf.Abs(joystick.Horizontal) + 1);
						}
						else {
				...
				//horizontal movement
				if (joystick.Horizontal > 0 && currentlyClimbingWall && climbMovementSpeed2 > 0 && (!reachedRightPoint || !stopAtSides2)){
				...
				else if (joystick.Horizontal < 0 && currentlyClimbingWall && climbMovementSpeed2 > 0 && (!reachedLeftPoint || !stopAtSides2)){
				...
				//vertical movement
				if (joystick.Vertical > 0 && currentlyClimbingWall && climbMovementSpeed2 > 0 && !reachedTopPoint){
				...
				else if (joystick.Vertical < 0 && currentlyClimbingWall && climbMovementSpeed2 > 0 && downCanBePressed && !reachedBottomPoint){
				...
				if (currentlyClimbingWall && climbMovementSpeed2 > 0){
					...
					if (joystick.Vertical != 0 && climbVertically2 && climbedUpAlready){
						if ((!reachedTopPoint || joystick.Vertical < 0) && (!reachedBottomPoint || joystick.Vertical > 0)){
					...
					if (joystick.Horizontal != 0 && climbHorizontally2 && (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis)){
	...
	void RotatePlayer () {
		...
		if ((directionVector.magnitude > 0 || (movement.sideScrolling.lockMovementOnXAxis || movement.sideScrolling.lockMovementOnZAxis)) && !currentlyClimbingWall && !turnBack && !back2){
			...
			float myAngle = Mathf.Atan2 (joystick.Horizontal, joystick.Vertical) * Mathf.Rad2Deg;
			...
			if (!currentlyOnWall){
				...
				else if ((!movement.firstPerson.walkBackwardsWhenDownKeyIsPressed || joystick.Vertical >= 0 || joystick.Horizontal != 0 || !inFirstPersonMode) && playerCamera.transform.parent != transform){
	...
	void GettingMovementDirection () {
		...
		if (grounded.currentlyGrounded && (noCollisionTimer < 5 || Physics.Raycast(pos, Vector3.down, maxGroundedDistance2, noWaterCollisionLayers))) {
			...
			if (!jumpPerformed){
				...
				moveDirection = new Vector3(joystick.Horizontal, moveDirection.y, joystick.Vertical);
			}
			...
			if (directionVector.magnitude != 0){
				...
				if ((!movement.firstPerson.walkBackwardsWhenDownKeyIsPressed || joystick.Vertical >= 0 || joystick.Horizontal != 0 || !inFirstPersonMode) && playerCamera.transform.parent != transform){
					//if the player rotates by pressing the arrow keys/moving the joystick
					if (!movement.firstPerson.onlyRotateWithCamera || !inFirstPersonMode){
	...
	void Swim2 () {
		...
		if (hPositive){
			hAxis = (Mathf.Abs(joystick.Horizontal) - Mathf.Abs(joystick.Vertical) + 1)/2;
			if (hAxis != 0){
				hAxis2 = hAxis*2 - 1;
			}
			else {
		...
		if (vPositive && hPositive && (joystick.Horizontal > 0 && joystick.Vertical > 0 || joystick.Vertical > 0 && joystick.Horizontal > 0)
		|| !vPositive && !hPositive && (joystick.Horizontal < 0 && joystick.Vertical < 0 || joystick.Vertical < 0 && joystick.Horizontal < 0)
		|| vPositive && !hPositive && (joystick.Horizontal > 0 && joystick.Vertical < 0 || joystick.Vertical > 0 && joystick.Horizontal < 0)
		|| !vPositive && hPositive && (joystick.Horizontal < 0 && joystick.Vertical > 0 || joystick.Vertical < 0 && joystick.Horizontal > 0)){
			hAxis2 = hAxis;
		}
		...
		if (vPositive){
			vAxis = (Mathf.Abs(joystick.Vertical) - Mathf.Abs(joystick.Horizontal) + 1)/2;
			if (vAxis != 0){
				vAxis2 = vAxis*2 - 1;
			}
			else {
	...
	void Swim3 () {
		...
		|| (playerCamera.GetComponent<CameraController>() && !playerCamera.GetComponent<CameraController>().goneThroughFirstPersonUpdate)) {
			...
			if (!swimming.invertXRotation){
				xDeg += joystick.Horizontal * swimming.swimRotationSpeed * 0.3f;
			}
			...
			if (!swimming.invertYRotation){
				yDeg += joystick.Vertical * swimming.swimRotationSpeed * 0.2f;
			}
	...
	void CheckClimbableEdges () {
		...
		if (!pullingUp){
			...
			&& Physics.Linecast(transform.position + bottomNoSurfaceDetectorHeight2 + transform.right/3 + topAndBottomNoSurfaceDetectorsWidth2 + transform.up*0.1875f, transform.position + bottomNoSurfaceDetectorHeight2 + transform.right/3 + topAndBottomNoSurfaceDetectorsWidth2 + (transform.forward*(climbingSurfaceDetectorsLength2 + 1)) + transform.up*0.1875f, out hit, noWaterCollisionLayers) && hit.transform.tag != climbableTag2){
				...
				if (currentlyClimbingWall && dropOffAtBottom2 && transform.rotation == lastRot2 && wallIsClimbable && joystick.Vertical < 0){
					moveDirection = Vector3.zero;
					swimDirection = Vector3.zero;
					swimmingMovementSpeed = 0;
					swimmingMovementTransferred = true;
			...
			Physics.Linecast(colCenter, colTop + transform.up/6, out hit, noWaterCollisionLayers)){
				...
				if (aboveTop){
					if (currentlyClimbingWall && pullUpAtTop2 && transform.rotation == lastRot2 && wallIsClimbable && joystick.Vertical > 0
	...
	void WallClimbingRotation () {
		...
		if (currentlyClimbingWall && !pullingUp){
			...
			if ((transform.rotation == lastRot3 || axisChanged) && (climbingMovement > 0 || joystick.Horizontal != 0) || hasNotMovedOnWallYet){
				
				//to the right of player
				if ((joystick.Horizontal > 0 || transform.rotation != lastRot2) && (wallIsClimbable)){
				...
				else if ((joystick.Horizontal < 0 || transform.rotation != lastRot2) && (wallIsClimbable)){
					if (Physics.Linecast(transform.position + climbingSurfaceDetectorsUpAmount2 + (transform.up*(climbingSurfaceDetectorsHeight2 + 1))*0.5625f, transform.position + climbingSurfaceDetectorsUpAmount2 - transform.right/3 + (transform.up*(climbingSurfaceDetectorsHeight2 + 1))*0.5625f, out hit, noWaterCollisionLayers) && hit.transform != null && hit.transform.tag == climbableTag2){
						if (joystick.Horizontal < 0){
							rotationNormal = Quaternion.LookRotation(-hit.normal);
							rotationState = 4;
				...
				if (climbingMovement > 0 || joystick.Horizontal != 0 || joystick.Vertical != 0){
					hasNotMovedOnWallYet = false;
				}
			}
			
			//checking to see if player changed their direction from left to right/right to left
			if ((joystick.Horizontal > 0 && horizontalAxis <= 0 || joystick.Horizontal < 0 && horizontalAxis >= 0)){
				axisChanged = true;
			}
			else {
				axisChanged = false;
	...
	void CheckIfStuck () {
		...
		if (pushAgainstWallIfPlayerIsStuck2){
			...
			if (climbedUpAlready){
				...
				if (currentlyClimbingWall && climbingMovement > 0 && (joystick.Horizontal > 0 && (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis) || joystick.Horizontal < 0 && (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis) || joystick.Vertical > 0 || joystick.Vertical < 0) || pullingUp){
					...
					if (transform.position == lastPos){
						...
						if (noCollisionTimer < 5 ){
							...
							if (!hasNotMovedOnWallYet && (joystick.Horizontal > 0.1f && (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis) || joystick.Horizontal < -0.1f && (!movement.sideScrolling.lockMovementOnXAxis && !movement.sideScrolling.lockMovementOnZAxis)
							|| joystick.Vertical > 0.1f || joystick.Vertical < -0.1f)){
								stuckInSamePos = true;
							}
						}
					}
